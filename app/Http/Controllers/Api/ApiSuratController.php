...
class ApiSuratController extends Controller
{
    ...
    */
    ...
    public function mainKey($z, $mode = 'ECB', $iv = null)
    {
    ...
    **/
    public function decrypt($y)
    {
        ...
        $decryptisi = rtrim($x, chr(0)); // Remove any buffer residue on return.
        return $decryptisi;
    }

    public function decryptfile($y)
    {
        $t = array(); // 16-byte block
        $x = ''; // returned plain text;
        $y_block = $this->iv;
        ...
        $filedecrypt = rtrim($x, chr(0)); // Remove any buffer residue on return.
        return $filedecrypt;
    }
    ...
    **/
    ...
    {
        $pengguna = DB::table('users')->select('email', 'id')->get();
        $surat = DB::table('surats')->join('users','receive','=','users.id')->join('pins', 'surats.to_id', '=','pins.user_id')->select('surats.*','users.email','pins.pin')->where('surats.user_id', $id)->get();
        if ($surat->isEmpty()) {
            $response = [
                'message' => 'true',
                'pengguna' => $pengguna
    ...
    {
        $surat = DB::table('surats')->join('users','surats.receive','=','users.id')->join('pins', 'surats.to_id', '=','pins.user_id')->select('surats.*','pins.pin')->where('surats.to_id', $id)->get();
        if ($surat->isEmpty()) {
            $response = [
                'message' => 'true',
            ];    
    ...
    {
        $surat = DB::table('surats')->join('users','to_id','=','users.id')->join('pins', 'surats.to_id', '=','pins.user_id')->select('pins.pin')->where('surats.id', $id)->get();
        if ($surat->isEmpty()) {
            $response = [
    ...
    public function prosesOut($id)
    {
        $surat = DB::table('surats')->join('users','to_id','=','users.id')->join('pins', 'surats.user_id', '=','pins.user_id')->select('pins.pin')->where('surats.id', $id)->get();
        if ($surat->isEmpty()) {
            $response = [
                'message' => 'Maaf data tidak ada',
            ];    
    ...
    {
        $surat = DB::table('surats')->join('users','to_id','=','users.id')->select('surats.*','users.email as rec')->where('surats.id', $id)->get();
        foreach ($surat as $data) {
            $dekIsi = Crypt::decrypt($data->isi);
            $dekFile = Crypt::decrypt($data->file);
            $newpin = Crypt::decrypt($data->keterangan);
            $encPin = $this->mainKey($newpin);
            $aesDekIsi = $this->decrypt($dekIsi);
            $aesDekFile = $this->decryptfile($dekFile);
        }
    ...
    {
        $surat = DB::table('surats')->join('users','user_id','=','users.id')->select('surats.*','users.email')->where('surats.id', $id)->get();
        foreach ($surat as $data) {
            $dekIsi = Crypt::decrypt($data->isi);
            $dekFile = Crypt::decrypt($data->file);
            $newpin = Crypt::decrypt($data->keterangan);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $pin_id = Pin::where('user_id', request('user_id'))->first();
            if (!$pin_id) {
                return response()->json([
                    'status' => 'PIN Salah !!!'
                ], 200);
            } else {
                $newpin = request('pin').'0608199714';
                $enkpin = Crypt::encrypt($newpin);
                $encPin = $this->mainKey($newpin);
                $encIsi = $this->encrypt(request('isi'));
                $encFile = $this->encryptfile($path);
                $saveEncIsi = Crypt::encrypt($encIsi);
                $saveEncFile = Crypt::encrypt($encFile);
                Surat::create([
                    'user_id' => request('user_id'),
                    'to_id' => request('to_id'),
                    'email' => request('email'),
                    'receive' => request('to_id'),
    ...
    {
        $data = $request->all();
        $surat = Surat::where('id', $data['id'])->update([
            'to_id' => 0
        ]);
        if ($surat) {
            return response()->json([
