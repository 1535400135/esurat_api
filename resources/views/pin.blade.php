        ...
        <div class="card-body row">
          ...
          <div class="col-7">
            ...
            <div class="form-group">
              <label for="inputName">PIN Lama</label>
              <input type="password" id="pin_old" name="pin_old" class="form-control" />
            </div>
            <div class="form-group">
...
<script>
  ...
  $('#formPin').submit(function(e) {
    ...
    $.ajax({
      type:'POST',
      url:'http://127.0.0.1:8000/api/pin/update',
      data:formData,
      cache:false,
      contentType:false,
      processData:false,
      success:(data) => {
        ...
        if (data.status==='Password Salah') {
            ...
            htmls+='<h5><i class="icon fas fa-ban"></i> Password Salah</h5>';
            htmls+= '</div>';
            $('#alert').append(htmls);
            htmls='';
        } else if(data.status==='PIN Tidak Sama') {
            var htmls='<div class="alert alert-danger alert-dismissible">';
            htmls+='<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>';
            htmls+='<h5><i class="icon fas fa-ban"></i> PIN Tidak Sama!</h5>';
            htmls+= '</div>';
            $('#alert').append(htmls);
            htmls='';
            ...
            var htmls='<div class="alert alert-danger alert-dismissible">';
            htmls+='<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>';
            htmls+='<h5><i class="icon fas fa-ban"></i> PIN Lama Salah!</h5>';
            htmls+= '</div>';
            $('#alert').append(htmls);
            htmls='';
        } else {
